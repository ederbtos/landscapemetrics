...
(_build_transition_matrix/_project_future_landcover,
backend/app/services/landscape_core.py) — ver "Predição de anos futuros
(cadeia de Markov)" no ROADMAP.md. Movidas de app.py (Streamlit, removido)
para landscape_core.py durante a migração para o backend — sem endpoint/UI
próprio ainda no backend (ver ROADMAP.md), só a lógica pura + seus testes.
"""
import sys
...
def test_build_transition_matrix_all_class1_to_class2():
    ...
    transition = landscape_core._build_transition_matrix([arr_2000, arr_2010], [2000, 2010])
...
def test_build_transition_matrix_aggregates_multiple_consecutive_pairs():
    ...
    transition = landscape_core._build_transition_matrix([arr_2000, arr_2010, arr_2020], [2000, 2010, 2020])
...
def test_build_transition_matrix_orders_by_year_regardless_of_input_order():
    ...
    transition = landscape_core._build_transition_matrix([arr_2010, arr_2000], [2010, 2000])
...
def test_project_future_landcover_full_transition_one_step():
    ...
    result = landscape_core._project_future_landcover(transition_df, 2010, last_proportions, 10.0, [2020])
...
def test_project_future_landcover_identity_matrix_keeps_proportions_stable():
    ...
    result = landscape_core._project_future_landcover(transition_df, 2010, last_proportions, 10.0, [2020, 2030])
...
def test_project_future_landcover_ignores_non_future_years():
    ...
    result = landscape_core._project_future_landcover(transition_df, 2010, last_proportions, 10.0, [2000, 2010])
...
def test_project_future_landcover_sums_to_100_percent():
    ...
    result = landscape_core._project_future_landcover(transition_df, 2010, last_proportions, 10.0, [2015])
